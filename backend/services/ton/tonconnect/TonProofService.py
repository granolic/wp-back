...
class TonProofService(TonProofServiceInterface):
    ...
    async def _check_payload(self, request: CheckProofRequest) -> VerifyResult:
        ...
        try:
            ...
            public_key = self.parse_wallet_public_key(request_raw.init_state.code, request_raw.data)
        ...
        wanted_address = await self.ton_client.get_account_address(request_raw.init_state)
    ...
    def detect_address_info(self, address: str) -> TonAddressInfo:
        detected_address_info = detect_address(address)
        wc = detected_address_info['raw'].split(":")[0]

        return TonAddressInfo(
            workchain=wc,
            bouncable='bouncable' in detected_address_info['given_type'],
            testnet_only=detected_address_info['test_only'],
        )
